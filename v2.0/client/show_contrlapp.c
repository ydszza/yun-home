...
//开关图片资源数组
char* bmp_name[] = {"./bmp/air_on.bmp","./bmp/air_off.bmp",
					"./bmp/curtain_on.bmp","./bmp/curtain_off.bmp",
					"./bmp/light_on.bmp","./bmp/light_off.bmp"};
...
int show_contrlapp(void)
{
	...
	//显示默认关闭空调图标	
	show_bmp(bmp_name[1], 200, 180, 80, 60);
	//show_bmp(500,200,40,40,bmp_name[3]);
	//显示默认关闭窗帘图标	
    show_bmp(bmp_name[3], 200, 360, 80, 60);
	//show_bmp(500,300,40,40,bmp_name[5]);
	//显示默认关闭灯图标	
    show_bmp(bmp_name[5], 200, 530, 80, 60);
	// //显示退出图标
    // show_bmp(bmp_name[6], 400,500,40,40);

	//开启一个线程初始化socket，连接服务器同步家居状态并接收控制命令
    int sock_fd; 
	pthread_t thread;
    int ret = pthread_create(&thread, NULL, (void *)sync_stat, &sock_fd);  
...
void sync_stat(void *sock_fd_t)
{
	int sock_fd = *((int *)sock_fd_t);
	sock_fd = socket_init(PORT, IPADDR);
    if(sock_fd < 0){
        ...
        //显示一秒后删除
        sleep(1);
        lcd_draw_block(50,200, 280,30, 0x00ffffff);
    }
    //发送控制家电标志及初始家电状态
	unsigned char buf[4];
	buf[0] = 2;
	...
	while(1) {
		...
		buf[2] = flag_curtain;
		buf[3] = flag_light;
		ret = send(sock_fd, buf+1, 3, 0);
		if(ret < 0){
			perror("send local status failed");
        	//lcd提示语
        	show_bmp("./bmp/connect_error.bmp", 50,200,280,30);
			 //显示一秒后删除
        	sleep(1);
