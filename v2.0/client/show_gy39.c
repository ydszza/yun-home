...
void show_gy39()
{
    ...
    //初始化socket，连接服务器准备发送数据
    int sock_fd = socket_init(PORT, IPADDR);
    if(sock_fd < 0){
    ...
    while(1) {
        ...
        //获取数据
        ret = get_GY39_data(&gy39_data);

        //发送数据至服务器
        ret = send(sock_fd, &gy39_data, sizeof(gy39_data), 0);
        if(ret < 0){
            perror("send smoke data failed");
            //lcd提示语
            show_bmp("./bmp/connect_error.bmp", 50,200,280,30);
